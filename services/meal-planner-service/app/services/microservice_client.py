...
class MicroserviceClient:
    ...
    def __init__(self):
        ...
        self.recipe_service_url = os.getenv("RECIPE_SERVICE_URL", "http://recipe-service:8000")
    ...
    async def get_recipes(self) -> List[Dict[str, Any]]:
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                response = await client.get(f"{self.recipe_service_url}/api/recipes/")
    ...
    async def get_recipe(self, recipe_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                response = await client.get(f"{self.recipe_service_url}/api/recipes/{recipe_id}")
                response.raise_for_status()
                return response.json()
        except httpx.HTTPError as e:
