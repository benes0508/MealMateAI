...
const Recipes = () => {
  ...
  // Helper function to infer dietary info for CSV recipe
  const inferRecipeDietaryInfo = (recipe: any) => {
    const ingredients = recipe.ingredients?.toLowerCase() || '';
    const recipeTags = recipe.cuisine_path?.toLowerCase() || '';
    const description = recipe.description?.toLowerCase() || '';
    
    // Simple logic to infer dietary preferences
    const isVegetarian = !(/\b(chicken|beef|pork|meat|turkey|bacon|ham|sausage)\b/i.test(ingredients));
    
    const isVegan = isVegetarian && !(/\b(cheese|milk|cream|egg|honey|butter|yogurt)\b/i.test(ingredients));
    
    const hasGluten = /\b(wheat|flour|pasta|bread|gluten)\b/i.test(ingredients);
    const isGlutenFree = !hasGluten || /gluten[ -]free/i.test(recipeTags) || /gluten[ -]free/i.test(description);
    
    const isDairyFree = !(/\b(milk|cheese|cream|butter|yogurt|dairy)\b/i.test(ingredients));
    
    const isNutFree = !(/\b(nut|almond|walnut|peanut|cashew|pecan|hazelnut)\b/i.test(ingredients));
    
    const hasHighCarbs = /\b(sugar|pasta|bread|rice|potato|flour)\b/i.test(ingredients);
    const isLowCarb = !hasHighCarbs || /low[ -]carb/i.test(recipeTags) || /keto/i.test(recipeTags) || /low[ -]carb/i.test(description);
  ...
  // Function to filter CSV recipes based on dietary preferences
  const filterCsvRecipesByDietary = (recipes: any[], filters: typeof dietaryFilters) => {
    const activeFilters = Object.entries(filters).filter(([_, value]) => value);
    
    // If no filters are active, return all recipes
    if (activeFilters.length === 0) {
      return recipes;
    }
    
    return recipes.filter(recipe => {
      // For each recipe, infer its dietary info
      const dietaryInfo = inferRecipeDietaryInfo(recipe);
      
      // Check if recipe matches all selected dietary filters
      return activeFilters.every(([key, _]) => {
        return dietaryInfo[key as keyof typeof dietaryInfo];
      });
    });
  };
  ...
  useEffect(() => {
    const fetchRecipes = async () => {
      try {
        ...
        // Always use CSV data if we have it, regardless of filters
        const isUsingCsvData = !!storedCsvRecipes;

        if (isUsingCsvData) {
          // We already have CSV data, apply filters and paginate client-side
          setRecipeSource('csv');
          ...
          // Filter recipes based on search query, selected tags and dietary preferences
          ...
          // Apply dietary filters to CSV data
          filteredRecipes = filterCsvRecipesByDietary(filteredRecipes, dietaryFilters);
          ...
          const recipes = paginatedRecipes.map((csvRecipe: any) => {
            const dietaryInfo = inferRecipeDietaryInfo(csvRecipe);
          ...
          // Try to get recipes from database first
          try {
            ...
            // If database has no results, load CSV recipes as fallback
            else {
              // Load CSV recipes automatically with all filters applied
              await loadCsvRecipes(page);
            }
          } catch (err) {
            console.error('Failed to fetch recipes from database:', err);
            // Fallback to CSV recipes
            await loadCsvRecipes(page);
          }
        }
      } catch (err) {
  ...
  const loadCsvRecipes = async (currentPage = 1) => {
    try {
      ...
      // Check if we need to filter by search query, tags, or dietary preferences
      ...
      // Transform the CSV data to match Recipe type
      const recipes = paginatedRecipes.map((csvRecipe: any) => {
        // Get inferred dietary info for each recipe
        const dietaryInfo = inferRecipeDietaryInfo(csvRecipe);
